<template>
	<Card>
		<template #title>
			<h1 class="text-color text-2xl font-semibold">{{ $t("auth.verifyCode.title") }}</h1>
		</template>

		<template #subtitle>
			<p class="mb-4">{{ $t("auth.verifyCode.subtitle") }}</p>
		</template>
				...
				<div class="flex flex-col items-center space-y-4">
					...
					<div>
						<InputOtp
							v-model="form.code"
							:invalid="hasError"
							:disabled="verifyCodeStatus === 'pending'"
							:length="OTP_LENGTH"
							integerOnly
							fluid />
					</div>
					...
					<div class="inline-flex flex-wrap items-center justify-center gap-x-1">
						<span class="text-muted-color text-sm">{{ $t("auth.verifyCode.labels.helper") }}</span>
						<Button
							@click="onResendCode"
							variant="link"
							class="!p-0"
							size="small"
							:disabled="!!resendCountdownTimer">
					...
					<div class="flex justify-center">
						<Button
							...
							:label="$t('auth.verifyCode.buttons.back')"
							variant="link"
							class="!p-0"
							size="small" />
					</div>
...
<script lang="ts" setup>
	import { OtpType } from "#shared/utils/otp-type";
